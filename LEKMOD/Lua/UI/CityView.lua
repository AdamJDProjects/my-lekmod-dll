...
local GetGoldenAgePointsTooltip = EUI.GetGoldenAgePointsTooltip -- v34
...
--			tips:insertLocalizedIfNonZero( "TXT_KEY_STAT_POSITIVE_YIELD_MOD", "[ICON_HEALTH_1]", healthModifier )
		...
		for yieldID = 0, YieldTypes.NUM_YIELD_TYPES-1 do
			...
			-- MOD.EAP START
			-- Tech Enhanced Yield support

			for row in GameInfo.Building_TechEnhancedYieldChanges( thisBuildingAndYieldTypes ) do
				local techEnhancedYield = row.Yield or 0

				-- Check if the team has the required tech instead of the player
				local teamHasTech = false
				if building.EnhancedYieldTech then
					local techID = GameInfoTypes[building.EnhancedYieldTech]
					if techID then
						teamHasTech = Teams[cityOwner:GetTeam()]:IsHasTech(techID)
					end
				end
				
				if techEnhancedYield ~= 0 and teamHasTech then
					buildingYieldRate = buildingYieldRate + techEnhancedYield
				end
			end
...
local function PlotButtonForceClicked( plotIndex )
	local city = GetSelectedModifiableCity()
	local plot = city and city:GetCityIndexPlot( plotIndex )
	if plot then
		local outside = city ~= plot:GetWorkingCity()
			...
			if plot and plot:GetOwner() == cityOwnerID then
				...
				if iconID and showButtons then
					...
					if not cityPlotIndex or g_isViewingMode then
						...
						button:RegisterCallback( Mouse.eRClick, PlotButtonForceClicked )
					end
				end
			end
		end --loop
...
--todo BE
		...
		elseif focusType == CityAIFocusTypes.CITY_AI_FOCUS_TYPE_GOLDEN_AGE_POINTS then -- v34
			Controls.GoldenAgeFocusButton:SetCheck( true ) 
		else
			Controls.BalancedFocusButton:SetCheck( true )
		end
...
-- todo
		...
		-- Display Golden Age Points v34
		local iGoldenAgePointsPerTurn = city:GetYieldRateTimes100(YieldTypes.YIELD_GOLDEN_AGE_POINTS) / 100
		Controls.GoldenAgePointsPerTurnLabel:LocalizeAndSetText( "TXT_KEY_CITYVIEW_PERTURN_TEXT", iGoldenAgePointsPerTurn )
		...
		Controls.GoldenAgeFocusButton:SetDisabled( g_isViewingMode ) -- v34
...
local g_toolTips = {
	...
	GoldenAgeBox = function() -- v34
		return SetToolTipString( GetGoldenAgePointsTooltip )
	end,
	TourismBox = function()
...
local g_callBacks = {
	...
	GoldenAgeFocusButton = FocusButtonBehavior,-- v34
...
Controls.GoldenAgeFocusButton:SetVoid1( CityAIFocusTypes.CITY_AI_FOCUS_TYPE_GOLDEN_AGE_POINTS ) -- v34
